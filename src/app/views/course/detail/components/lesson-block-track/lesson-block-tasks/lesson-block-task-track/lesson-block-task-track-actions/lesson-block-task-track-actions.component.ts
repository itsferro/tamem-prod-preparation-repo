...
export class LessonBlockTaskTrackActionsComponent implements OnInit {
  ...
  actionFieldConfigs: { [key: string]: any } = {
    'assign-task': {
      visible: ['assignedToUserId', 'dueDate', 'comment', 'sendNotification','noApprovalNeeded'],
  ...
  defaultFieldConfig = {
    visible: ['comment', 'sendNotification' , 'noApprovalNeeded'],
    required: ['comment']
  };
  ...
  initializeFieldVisibility(): void {
    const allFields = [
      ...
      'comment', 'sendNotification', 'markAsUrgent', 'requiresApproval','noApprovalNeeded',
      // 🆕 ADD THESE TWO LINES
